...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
float pid_p_gain_roll = 3.6;               //Gain setting for the roll P-controller
float pid_i_gain_roll = 0.002;              //Gain setting for the roll I-controller
float pid_d_gain_roll = 22.0;              //Gain setting for the roll D-controller
...
float pid_i_gain_yaw = 0.0005;               //Gain setting for the pitch I-controller. //0.02
float pid_d_gain_yaw = 2.0;                //Gain setting for the pitch D-controller.
...
byte eeprom_data[36];                                                 //this will be used to read IMU data
...
//volatile because this value can change at anytime
//int counter_channel_1, counter_channel_2, counter_channel_3, counter_channel_4, loop_counter;//don't need
...
//    receiver_input_channel_4 = convert_receiver_channel(4);                 //Convert the actual receiver signals for yaw to the standard 1000 - 2000us.
    ...
    count=  (count+1)%11;
    acc_axis[1] = filter(Wire.read()<<8|Wire.read(),1);                               //Add the low and high byte to the acc_x variable.
    acc_axis[2] = filter(Wire.read()<<8|Wire.read(),2);                               //Add the low and high byte to the acc_y variable.
    acc_axis[3] = filter(Wire.read()<<8|Wire.read(),3);                                //Add the low and high byte to the acc_z variable.
